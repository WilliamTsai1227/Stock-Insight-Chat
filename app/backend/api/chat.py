...
async def get_ai_response(
    ...
    async def _rewrite_search_query(
        ...
        2. 其他輸入：進 LLM 改寫，依「是否需即時/查證」決定搜尋詞或 __NO_SEARCH__
        3. Fallback：LLM 失敗時不搜尋（保守策略，避免誤打 Tavily）
        ...
        try:
            ...
            system = (
                ...
                "根據對話歷史與使用者最新輸入，**僅在必要時**才建議網路搜尋；預設傾向 __NO_SEARCH__。\n\n"
                + long_hint +
                "【原則】必要時才搜：能用常識、推理、對話上下文或模型既有知識回答的，一律 __NO_SEARCH__。\n"
                "只有需要**即時、可變、需查證**的外部事實時，才輸出搜尋詞。\n\n"
                "【一律 __NO_SEARCH__ — 不需搜尋】\n"
                "- 常識、概念解釋、定義、原理說明\n"
                "- 程式撰寫/除錯、演算法、SQL、技術教學（除非明確問「最新版本/剛發布」）\n"
                "- 數學計算、推導、邏輯題\n"
                "- 翻譯、改寫、潤稿、寫作、摘要、列點整理\n"
                "- 純聊天（感謝、確認、情緒、閒聊）\n"
                "- 要求延續/展開上文 AI 已給的分析、建議、大綱（如「再詳細說」「繼續」且上文非找資料）\n\n"
                "【才輸出搜尋詞 — 需要即時或查證】\n"
                "- 時事、新聞、近期事件\n"
                "- 票價、房價、匯率、股價等會變動的數字\n"
                "- 活動、展覽、演唱會、營業時間、是否仍開放\n"
                "- 最新政策、法規、公告、官方規定\n"
                "- 明確要求「最新」「最近」「現在」「查一下」「幫我找」等需查證的事實\n"
                "- 餐廳/店家/行程推薦、地點資訊（需外部資料時）\n\n"
                "判斷規則：\n"
                "1. 若屬【不需搜尋】類別 → __NO_SEARCH__。\n"
                "2. 若屬【需要即時或查證】且無法從對話歷史直接回答 → 輸出最佳搜尋詞（繁體中文，10–25 字）。\n"
                "3. 若使用者輸入是對上文主題的補充細節（天數、預算、地點偏好），"
                "   且上文主題本身需要外部資料 → 把核心主題與新細節合成搜尋詞。\n"
                "   - 例：上文問福岡旅遊，使用者說「5天」→ 福岡 5天自由行 行程規劃\n"
                "   - 例：上文問台北咖啡廳，使用者說「有插座的」→ 台北 咖啡廳 有插座 適合工作\n"
                "4. 若使用者輸入是模糊的續接詞（如「再來」、「繼續」、「還有呢」）：\n"
                "   - 上文 AI 已做完分析/整理，使用者要求繼續展開 → __NO_SEARCH__\n"
                "   - 上文在找資料/推薦，使用者要更多同類資料 → 用上文主題合成搜尋詞\n"
                "5. 若問題含「最近」「最新」「這週」「近期」等時效性用語且需外部資料，"
                f"   搜尋詞須含當前年份或月份（例如「{current_year} 演唱會 近期」）。\n\n"
                "重要：只輸出搜尋詞或 __NO_SEARCH__，不要輸出任何其他文字。\n\n"
                "範例：\n"
                "  輸入「用 Python 寫快速排序」→ __NO_SEARCH__\n"
                "  輸入「把這段翻成英文」→ __NO_SEARCH__\n"
                "  輸入「12×35 等於多少」→ __NO_SEARCH__\n"
                "  輸入「幫我潤飾這段履歷」→ __NO_SEARCH__\n"
            ...
            if query == "__NO_SEARCH__" or not query:
                return ""
            return query[:_TAVILY_MAX_CHARS]
        except Exception as e:
            print(f"[General/QueryRewrite] 改寫失敗，保守跳過搜尋: {e}")
            return ""
